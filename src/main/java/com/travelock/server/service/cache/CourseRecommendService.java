...
public class CourseRecommendService {
    ...
    public void updateTopFullCourses() {
        try {
            ...
            List<FullCourseCacheDto> cacheList = DTOConverter.toDtoList(topFullCourses, fullCourse -> new FullCourseCacheDto(
            ...
            // 객Dto체 리스트를 JSON 문자열로 변환
            String jsonData = objectMapper.writeValueAsString(cacheList);

            // Redis에 캐싱
            jsonRedisTemplate.opsForValue().set(REDIS_FULL_KEY, jsonData);
        } catch (Exception e) {
    ...
    public List<FullCourseCacheDto> getTopFullCoursesFromCache() {
        String jsonData = jsonRedisTemplate.opsForValue().get(REDIS_FULL_KEY);
        if (jsonData == null) {
            throw new ResourceNotFoundException("추천 전체일정을 찾을 수 없습니다.");
        }
        // 안전한 타입 변환을 위해 ObjectMapper 사용하여 JSON 역직렬화
        try {
            List<FullCourseCacheDto> cachedData = objectMapper.readValue(jsonData, new TypeReference<List<FullCourseCacheDto>>() {
            });
            return cachedData;
    ...
    public void updateTopDailyCourses() {
        try {
            ...
            List<DailyCourseCacheDto> cacheList = DTOConverter.toDtoList(topDailyCourses, dailyCourse -> new DailyCourseCacheDto(
            ...
            // Redis에 캐싱
            jsonRedisTemplate.opsForValue().set(REDIS_DAILY_KEY, jsonData);
        } catch (Exception e) {
    ...
    public List<DailyCourseCacheDto> getTopDailyCoursesFromCache() {
        String jsonData = jsonRedisTemplate.opsForValue().get(REDIS_DAILY_KEY);
        if (jsonData == null) {
            throw new ResourceNotFoundException("추천 일일일정을 찾을 수 없습니다.");
        }
        // 안전한 타입 변환을 위해 ObjectMapper 사용하여 JSON 역직렬화
        try {
            List<DailyCourseCacheDto> cachedData = objectMapper.readValue(jsonData, new TypeReference<List<DailyCourseCacheDto>>() {
            });
            return cachedData;
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
